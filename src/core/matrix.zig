...
pub const Mat44f = struct {
  ...
  pub inline fn col(index:i2) Vec4f 
  {
    switch(index){
  ...
  {
    self.mm[0] = self.mm[0].mul_vec4(self.col(0));
    self.mm[1] = self.mm[1].mul_vec4(self.col(1));
    self.mm[2] = self.mm[2].mul_vec4(self.col(2));
    self.mm[3] = self.mm[3].mul_vec4(self.col(3));
  }
  ...
  pub inline fn mul33(self:*Mat44f, v:Vec4f) Vec4f
  {
    ...
    return Vec4f.init(
      self.mm[0].dot(v) * invW, 
      self.mm[1].dot(v) * invW, 
      self.mm[2].dot(v) * invW, 
      0
    );
  }
